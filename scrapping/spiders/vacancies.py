...
from helpers import convert_month_name_to_month_num, find_technologies
...
class VacanciesSpider(scrapy.Spider):
    ...
    def _parse_vacancy_details(response: Response) -> dict:
        yield {
            ...
            "place": response.css(".place::text").get(),
            "description": [
            ...
            "required_technologies": find_technologies(
                "".join(
                    [
                        text.replace(u"\xa0", u" ").strip()
                        for text in response.css(
                        "div.b-typo.vacancy-section p::text, div.b-typo.vacancy-section ul li::text"
                    ).getall()
                    ]
                )
            )
